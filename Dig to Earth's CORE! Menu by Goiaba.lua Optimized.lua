...
local BloodDiamondSection = Tabs.OurWorld:AddSection("💎 ‣ BloodDiamond")

autoFire(Tabs.OurWorld, "Auto BloodDiamond", "Auto add BloodDiamond every 0s.", 0, function()
    return ReplicatedStorage:WaitForChild("Remotes",5):FindFirstChild("TreasureEvent")
end, function() return {"BloodDiamond"} end)

local GalaxyStarSection = Tabs.OurWorld:AddSection("🌠 ‣ GalaxyStar")

autoFire(Tabs.OurWorld, "Auto GalaxyStar", "Auto add GalaxyStar every 0s.", 0, function()
    return ReplicatedStorage:WaitForChild("Remotes",5):FindFirstChild("TreasureEvent")
end, function() return {"GalaxyStar"} end)

local ShardsStarSection = Tabs.OurWorld:AddSection("✨ ‣ Shards")

autoFire(Tabs.OurWorld, "Auto Shards", "Auto add Shards every 0s.", 0, function()
    return ReplicatedStorage:WaitForChild("Remotes",5):FindFirstChild("TreasureEvent")
end, function() return {"LightShard"} end)

local CollectionSection = Tabs.OurWorld:AddSection("📦 ‣ Collections")

Tabs.OurWorld:AddButton({
    Title = "Claim All Collections",
    Description = "",
    Callback = function()
        task.spawn(function()
            local collections = {
            ...
            for i = 1, 5 do
                for _, col in ipairs(collections) do
                    ...
                    game:GetService("ReplicatedStorage")
                        ...
                        :WaitForChild("TreasureEvent")
                        :FireServer(unpack(args))

                    task.wait(0.5) -- espera 0.5s ANTES de ir para a próxima coleção
                end
            end
        end)
    end
